...
#CUDA_VISIBLE_DEVICES=0 python finetune.py exp_name=grasp_right_ft_abs agent.features.restore_path=/grogu/user/rmendonc/data4robotics_features/visual_features/vit_base/SOUP_1M_DH.pth buffer_path=/grogu/user/rmendonc/alfred_data/grasp_right/absolute/buf.pkl task=single_hand ac_chunk=16

# CUDA_VISIBLE_DEVICES=0 python finetune.py agent=diffusion_unet exp_name=grasp_right_dp_abs \
# buffer_path=/grogu/user/rmendonc/alfred_data/grasp_right/absolute/buf.pkl \
# task=single_hand trainer=bc_cos_sched ac_chunk=16 \
# train_transform=medium task.train_buffer.cam_indexes=[0] agent.features.feature_dim=256 max_iterations=500000
